    ...
    <div class="loginInner">
      <div class="login_header">
        ...
        <div class="login_header_title">
          <a href="javascript:;" :class="{on: loginWay}" @click="loginWay=true">邮箱登录</a>
        ...
        <form @submit.prevent="login">
          <div :class="{on: loginWay}">
            <section class="login_message">
              <input type="text" placeholder="邮箱地址" v-model="email">
              <button
                :disabled="!rightEmail"
                class="get_verification"
                :class="{right_email: rightEmail}"
            ...
            <section class="login_hint">
              温馨提示：未注册硅谷外卖帐号的邮箱号，登录时将自动注册，且代表已同意
...
<script>
  ...
  export default {
    ...
    computed:{
      rightEmail(){
        return /^([a-zA-Z0-9]+[_|\_|\.]?)*[a-zA-Z0-9]+@([a-zA-Z0-9]+[_|\_|\.]?)*[a-zA-Z0-9]+\.[a-zA-Z]{2,3}$/.test(this.email)
      }
    },
    methods:{
      //异步获取邮箱验证码
      ...
      async login(){
        ...
        if(this.loginWay){  //短信验证
          const {rightEmail, email, code} = this
          if(!rightEmail){
            //邮箱不正确
          ...
          //发送ajax请求，邮箱登录
          result = await reqSmsLogin(email,code)
        }else{  //密码验证
